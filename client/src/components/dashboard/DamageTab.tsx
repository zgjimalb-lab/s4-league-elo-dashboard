...
export default function DamageTab({
  ...
  const chartData = useMemo(() => {
    ...
    return playerStats
      ...
      .map((stat) => ({
        ...
        avgDamage: Math.round(stat.avg_damage || 0),
      }))
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Durchschnittlicher Damage pro Match</CardTitle>
        </CardHeader>
        <CardContent>
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={500}>
              <BarChart data={chartData}>
                ...
                <YAxis 
                  label={{ value: 'Avg Damage', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip />
                <Legend />
                <Bar dataKey="avgDamage" name="Avg Damage" radius={[8, 8, 0, 0]}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Damage-Statistiken</CardTitle>
        </CardHeader>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/50">
                  ...
                  <th className="text-right py-3 px-6 font-semibold text-sm">Gesamt Damage</th>
                  <th className="text-right py-3 px-6 font-semibold text-sm">Avg Damage</th>
                </tr>
              </thead>
              <tbody>
