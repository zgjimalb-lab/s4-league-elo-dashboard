...
export default function GoalsTab({
  ...
  const chartData = useMemo(() => {
    ...
    return playerStats
      ...
      .map((stat) => ({
        ...
        avgGoals: parseFloat(((stat.avg_goals || 0).toFixed(2))),
      }))
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Durchschnittliche Goals pro Match</CardTitle>
        </CardHeader>
        <CardContent>
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={500}>
              <BarChart data={chartData}>
                ...
                <YAxis 
                  label={{ value: 'Avg Goals', angle: -90, position: 'insideLeft' }}
                />
                <Tooltip />
                <Legend />
                <Bar dataKey="avgGoals" name="Avg Goals" radius={[8, 8, 0, 0]}>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Goals-Statistiken</CardTitle>
        </CardHeader>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border/50">
                  ...
                  <th className="text-right py-3 px-6 font-semibold text-sm">Gesamt Goals</th>
                  <th className="text-right py-3 px-6 font-semibold text-sm">Avg Goals</th>
                </tr>
              </thead>
              <tbody>
                {chartData.map((item, index) => {
                  ...
                  return (
                    <tr key={`goals-${index}-${item.player}`} className="border-b border-border/30 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="text-right py-4 px-6 text-base">{Math.floor(stat.total_goals || 0)}</td>
                      <td className="text-right py-4 px-6 text-base font-semibold">{item.avgGoals.toFixed(2)}</td>
                    </tr>
                  );
                })}
              </tbody>
